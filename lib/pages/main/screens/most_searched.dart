...
class MostSearchedVehiclesScreen extends StatefulWidget {
  final List<VehicleEntry> vehicles;

  const MostSearchedVehiclesScreen({
    Key? key,
    required this.vehicles,
  }) : super(key: key);

  @override
  State<MostSearchedVehiclesScreen> createState() =>
      _MostSearchedVehiclesScreenState();
}

class _MostSearchedVehiclesScreenState
    extends State<MostSearchedVehiclesScreen> {
  String _searchQuery = '';
  ...
  void _initializeVehicles() {
    final mobilVehicles = widget.vehicles
        .where((v) => v.fields.jenisKendaraan == JenisKendaraan.MOBIL)
        .take(12)
        .toList();
    final motorVehicles = widget.vehicles
        .where((v) => v.fields.jenisKendaraan == JenisKendaraan.MOTOR)
        .take(8)
        .toList();

    setState(() {
      _filteredVehicles = [...motorVehicles, ...mobilVehicles];
    });
  }
